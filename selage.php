<?php
    ...
    elseif (isset($_REQUEST['siscon_param'])){
        $_SESSION['usr_codage'] = $_REQUEST['siscon_param'];
        $_SESSION['usr_nomage'] = $_REQUEST['siscon_paramb'];
...
<html lang="<?php echo(($langles)?'en':'es');?>">
    <head>
        ...
        lagrid_datatable("selage","selage.php",1); // nom de la base, php de execució x clic, selecció de registre 1/0
        ?>
    </head>
    <body>
        <?php 
            ...
            menutct();
            // -- Page Title -->
            pagetitle("Selecci&oacute;n de agente ".miratct(),$_SESSION['usr_nomage'],"Selecci&oacute;n de agente ".miratct(),$_SESSION['usr_nomage'],"selage"); 
        ?>
        <div class="container">
            <div class="row" style="padding: 20px 0px 20px 20px;">
                <div class="col-sm-2 table-responsive text-left col-centered">
                                    <form action="selage.php" method="post" id="nou">
                                        <button type="submit" name="totes" class="btn btn-primary" value="totes">Mostrar todos</button>
                                    </form>
                </div>
