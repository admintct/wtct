...
jQuery(document).ready(function() {
    ...
    $(".btn-filelogo").on("click",function(){
...
//alert($('#elbarco').val().length);
    if ($("input[name='fechaentrega']").length > 0) {
    ...
    if ($("input[name='datarecull']").length > 0) {
    ...
    if ($("input[name='dataentrega']").length > 0) {
    ...
    $('input').keydown( function (event) { //event==Keyevent
        if ($( ".entersubmit" ).length){
            ...
            switch(event.which) {
            case 13: 
                if ($(this).hasClass('autocompletar') || $(this).hasClass('nicascr')){
                }
                else{
...
//                lesdates  = [{"data":"codi"},{"data":"bl"},{"data":"descpol"},{"data":"bultos"},{"data":"pes"},{"data":"refcli"}];
                ...
                break
            case 'wusuaris':
                lesdates  = [{"data":"codi","sClass":"text-center"},{"data":"clientoagent","sClass":"text-center","searchable":false},{"data":"usuari"},{"data":"email"},{"data":"codcli"},{"data":"nomcli"},{"data":"nomuser"},
                ...
                break
            case 'selage':
                lesdates  = [{"data":"codi"},{"data":"nom"},{"data":"pobage"}];
                elordre = [[1,"asc"]];
                quinaretorna = 0;
                paramb = "2";
                break
            case 'selempre':
                ...
                break
            case 'quefan':
                lesdates  = [{"data":"codi"},{"data":"ip"},{"data":"DT"},{"data":"pagina"},{"data":"accio"},{"data":"valor"},{"data":"atribut"}];
                elordre = [[2,"desc"]];
                quinaretorna = 0;
                paramb = "2";
                break
                    ...
                    case 'wusuaris':
                        switch( aData['clientoagent']){
                            case 'CL':
                                ...
                                break;
                            case 'AG':{$('td:eq(1)', nRow).css('background-color', 'darkorange');}break;
                            case 'TCT':{$('td:eq(1)', nRow).css('background-color', 'gold');}break;
                            }
                        break;
                    }
                }
...
//                            segon : $('#puntse_pais').val()
    ...
    //                   alert("ID:"+ui.item.id);
                        ...
                        $(that).val(ui.item.bo);
                        switch($(that).attr('id')){
                            case 'agents_codi': $('#agents_nom').val(ui.item.id); break;
                            case 'agents_nom': $('#agents_codi').val(ui.item.id); break;
                            ...
                            case 'puntse_punt': $('#puntse_punt').val(ui.item.id); $('#formexport').submit();break;
                            case 'puntsi_pais': $('#puntsi_pais').val(ui.item.id); break;
                            case 'puntsi_punt': $('#puntsi_punt').val(ui.item.id); $('#formimport').submit();break;
                            default:
                                break;
                        }               
                    }
...
//        });
    ...
    if (elgooglemaps == 1){
        /* Google maps */
        var position = new google.maps.LatLng(41.32497, 2.14077);
...
//                $(selector).scrollTop($('.highlighted:first').position().top);
                var n = $('.highlighted:first').offset().top;
//alert("Offset:"+n);
//                $('body').scrollTop(parseInt(n));
                window.scrollTo(5,n)
            }
            return true;
        }
