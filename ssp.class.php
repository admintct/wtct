...
class SSP {
	...
	static function filter ( $request, $columns, &$bindings )
	{
		...
		$where = (($GLOBALS['el_filtre'])?$GLOBALS['el_filtre'].' ':'');
		...
		if ( $where !== '' || $GLOBALS['el_filtre']) {
                    if ($GLOBALS['el_filtre']){
			$where = 'WHERE '.$GLOBALS['el_filtre'].(($where)?' and ':'').$where;
                    }
                    else{
			$where = 'WHERE '.$where;
                    }
	...
	static function bind ( &$a, $val, $type )
	{
                ...
                $tmp = datetosql(str_replace('%', '', $val));

                $key = ':binding_'.count( $a );
		$a[] = array(
			'key' => $key,
			'val' => (($tmp)?('%'.$tmp.'%'):strtolower($val)),
			'type' => $type
		);
		return $key;
	}
